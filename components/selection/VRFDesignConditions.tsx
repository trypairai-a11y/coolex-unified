...
import { ArrowLeft, ArrowRight, Home, Sun, Sparkles, SlidersHorizontal, Check, AlertTriangle } from "lucide-react";
...
const INDOOR_TYPES: {
...
}[] = [
  {
    ...
    label: "Ducted Split",
    description: "Concealed ducted indoor unit",
    ...
    <div className="w-full">
      ...
      <div className="flex justify-end pt-6">
        <Button
          ...
          disabled={!allConfigured || (view === "outdoor" && !outdoorRatioValid)}
          title={
            view === "outdoor" && !outdoorRatioValid
              ? `Combination ratio is out of range (${MIN_COMBINATION_RATIO}–${MAX_COMBINATION_RATIO}%). Pick a valid outdoor size to continue.`
              : undefined
          }
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div
              className={`rounded-lg border p-3 ${
                ratioOutOfRange ? "border-red-300 bg-red-50" : "border-[#E2E8F4] bg-white"
              }`}
            >
              <p
                className={`text-[10px] font-semibold uppercase tracking-wide ${
                  ratioOutOfRange ? "text-red-600" : "text-[#8894AB]"
                }`}
              >
                Combination Ratio
              </p>
              <p
                className={`text-[14px] font-bold mt-1 flex items-center gap-1 ${
                  ratioOutOfRange ? "text-red-600" : "text-[#0D1626]"
                }`}
              >
                {effectiveCombinationRatio.toFixed(0)}%
                {ratioOutOfRange && <AlertTriangle className="w-3.5 h-3.5" strokeWidth={2.5} />}
              </p>
              <p
                className={`text-[11px] mt-0.5 ${
                  ratioOutOfRange ? "text-red-600 font-medium" : "text-[#8894AB]"
                }`}
              >
                {effectiveCombinationRatio > MAX_COMBINATION_RATIO
                  ? "Over-connected — out of range"
                  : effectiveCombinationRatio < MIN_COMBINATION_RATIO
                  ? "Under-connected — out of range"
                  : "Within range"}
              </p>
            </div>
            <div className="rounded-lg border border-[#E2E8F4] bg-white p-3">
