 ...
 *
 * Transcribed directly from the COOLEX "PNGv Performance Data" catalogue
 * (PNGv_Performance_Data.xlsx). The catalogue publishes the range across two
 * rating sheets ("Set 1" and "Set 2") that cover overlapping model sizes at
 * different airflow ranges / operating configurations. Because the two sets are
 * NOT a single continuous airflow curve (at a shared airflow they report
 * different capacities), each set is kept as its OWN selectable model variant —
 * suffixed "-S1" / "-S2" — rather than merged into one non-physical curve.
 *
 * Each variant is indexed by:
 *   • Indoor air flow (CFM)        — 3 rows per variant
...
/** Catalogue footnote shown beneath the PNGv performance table. */
export const PNGV_RATING_NOTE =
  'Cooling capacities are based on 95 °F condenser ambient. Sets S1 and S2 are distinct catalogue rating configurations.';

export const PNGV_CFM_BY_MODEL: Record<string, number[]> = {
  'PNGv-076-S1': [2200, 2350, 2500],
  'PNGv-090-S1': [2600, 2800, 3000],
  'PNGv-130-S1': [4000, 4400, 4800],
  'PNGv-180-S1': [5100, 5700, 6300],
  'PNGv-240-S1': [6800, 7000, 7300],
  'PNGv-300-S1': [9000, 9200, 9500],
  'PNGv-076-S2': [2000, 2600, 3000],
  'PNGv-090-S2': [2500, 3000, 3400],
  'PNGv-120-S2': [3500, 4000, 4800],
  'PNGv-180-S2': [5200, 6000, 6800],
  'PNGv-240-S2': [6750, 7500, 8200],
  'PNGv-300-S2': [7600, 8500, 9500],
};
...
/**
 * Bilinear PNGv performance lookup on (airflow CFM × condenser ambient °F).
...
export function getPNGVPerformance(
  ...
  const cfmRow = RAW_PNGV[modelNumber];
...
/** Expose the raw per-model matrix for the performance panel. */
export function getPNGVMatrix(modelNumber: string): Record<string, Record<string, PNGVPerfPoint>> | null {
  const cfmRow = RAW_PNGV[modelNumber];
...
/** CFM rows for a given PNGv model number (clamps the airflow-basis lookup). */
export function getPNGVCfmRows(modelNumber: string): number[] {
  return PNGV_CFM_BY_MODEL[modelNumber] ?? [];
}

// Re-export raw table for tooling (not used by the app at runtime).
export const __RAW_PNGV = RAW_PNGV;
