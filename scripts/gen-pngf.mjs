// Generates lib/mock-data/pngf-performance.ts from /tmp/pngf-raw.json and runs
// physical-sanity validation. Run: node scripts/gen-pngf.mjs
import { readFileSync, writeFileSync } from 'node:fs';

const SRC = '/tmp/pngf-raw.json';
const OUT = new URL('../lib/mock-data/pngf-performance.ts', import.meta.url);

const raw = JSON.parse(readFileSync(SRC, 'utf8'));

const AMBIENT = ['95', '115', '118.4', '125'];
const DB_ORDER = ['84.2/66.2', '80/67', '74/62', '68/57'];
const DB_KEY = { '84.2/66.2': '84.2', '80/67': '80', '74/62': '74', '68/57': '68' };

// ── Validation ──────────────────────────────────────────────────────────────
const warnings = [];
const errors = [];

for (const [model, m] of Object.entries(raw)) {
  const cfms = m.airflows;
  if (cfms.length !== 3) errors.push(`${model}: expected 3 airflows, got ${cfms.length}`);
  for (const cfm of cfms) {
    const row = m.rows[String(cfm)];
    if (!row) { errors.push(`${model} ${cfm}: missing row`); continue; }
    for (const db of DB_ORDER) {
      const cell = row[db];
      if (!cell) { errors.push(`${model} ${cfm} ${db}: missing`); continue; }
      // sensible <= total; total ↓ and kW ↑ as ambient rises
      let prevTotal = Infinity, prevKw = -Infinity;
      for (const amb of AMBIENT) {
        const [tot, sen, kw] = cell[amb];
        if (sen > tot) warnings.push(`${model} ${cfm} ${db} @${amb}: sensible ${sen} > total ${tot}`);
        if (tot > prevTotal) warnings.push(`${model} ${cfm} ${db}: total rose ${prevTotal}→${tot} @${amb} (expected ↓ with ambient)`);
        if (kw < prevKw) warnings.push(`${model} ${cfm} ${db}: kW fell ${prevKw}→${kw} @${amb} (expected ↑ with ambient)`);
        prevTotal = tot; prevKw = kw;
      }
    }
    // total ↑ with airflow at fixed (db, ambient) — check the 80/67 @95 anchor
  }
  // airflow monotonicity on the 80/67 @ 95 total
  let prev = -Infinity;
  for (const cfm of cfms) {
    const tot = m.rows[String(cfm)]['80/67']['95'][0];
    if (tot < prev) warnings.push(`${model}: 80/67@95 total fell ${prev}→${tot} as airflow rose to ${cfm}`);
    prev = tot;
  }
}

// ── Emit RAW literal ──────────────────────────────────────────────────────────
function cell(c) { return `[${c[0]}, ${c[1]}, ${c[2]}]`; }

let body = '';
for (const [model, m] of Object.entries(raw)) {
  body += `  '${model}': {\n`;
  for (const cfm of m.airflows) {
    body += `    ${cfm}: {\n`;
    for (const db of DB_ORDER) {
      const cells = AMBIENT.map(a => cell(m.rows[String(cfm)][db][a])).join(', ');
      body += `      '${DB_KEY[db]}': [${cells}],\n`;
    }
    body += `    },\n`;
  }
  body += `  },\n`;
...
const cfmByModel = Object.entries(raw)
  .map(([model, m]) => `  '${model}': [${m.airflows.join(', ')}],`)
  .join('\n');

const file = `import type { Model } from '@/types/product';
...
console.log(`Wrote ${Object.keys(raw).length} models to pngf-performance.ts`);
